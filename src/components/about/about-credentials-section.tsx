...
export function AboutCredentialsSection({
  ...
  const credentials =
    page?.credentials?.length ? page.credentials : fallbackCredentials;
  const approach = page?.methodSteps?.length ? page.methodSteps : fallbackApproach;
        ...
        <div>
          <Reveal>
            <h2 className="text-4xl font-bold text-[var(--foreground)] sm:text-5xl">
              {page?.credentialsTitle || "A Holistic Approach, Grounded in Training"}
            </h2>
          </Reveal>

          <Reveal delay={0.08}>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--muted)] sm:text-xl">
              {page?.credentialsIntro ||
          ...
          <div className="mt-10 grid gap-4">
            {credentials.map((item, index) => (
              <Reveal key={`${item}-${index}`} delay={0.12 + index * 0.05}>
          ...
          <Reveal>
            <div className="rounded-[28px] bg-white p-8 shadow-[0_10px_30px_rgba(16,32,66,0.05)]">
              <span className="inline-flex rounded-full bg-[#fff1e7] px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-[var(--brand)]">
                {page?.methodEyebrow || "Our Method"}
              </span>

              <h3 className="mt-6 text-3xl font-semibold text-[var(--foreground)]">
                {page?.methodTitle || "How the work unfolds"}
              </h3>

              <div className="mt-8 space-y-5">
                {approach.map((step, index) => (
                  <Reveal key={`${step}-${index}`} delay={0.1 + index * 0.06}>
